...
int R4AVA07::send(uint8_t rs485_addr, uint8_t func, uint32_t data) {
    uint8_t request[8] = {0x00};
    uint8_t response[BUFFER_SIZE];
    int read_size;

    request[0] = rs485_addr;
    request[1] = func;
    *(u_int32_t *)(&request[2]) = htonl(data);
    *(u_int16_t *)(&request[6]) = calculateCRC(&request[0], 6);

    write(fd, request, sizeof(request));
    sleep(1);
    
    read_size = read(fd, response, BUFFER_SIZE);

#ifdef DEBUG
    std::ostringstream msg;
    msg << std::hex << std::setfill('0') << std::uppercase;
    for (int i = 0; i < (int) sizeof(request); i++) {
        msg << std::setw(2)
    ...
    if (read_size <= 0){
        ...
        for (int i = 0; i < response[0]/2; i++) {
            // Read from the 3rd byte, read 2 bytes each.
            int pos = 2*i+3;
            read_data[i] = (uint16_t) response[pos] << 8 | response[pos+1];
        }
    }
    else {
